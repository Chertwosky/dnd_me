...
export function GameRoomPage({ roomId }: { roomId: string }) {
  ...
  >({});
  const [gmPanelOrder, setGmPanelOrder] = useState<Array<"admin" | "tokens">>([
    "admin",
    "tokens",
  ]);
  const [gmRightPanelOrder, setGmRightPanelOrder] = useState<
    Array<"party" | "initiative" | "tools">
  ...
  const handleMoveGmPanel = useCallback(
    (panelId: "admin" | "tokens", direction: "up" | "down") => {
  ...
  const handleMoveGmRightPanel = useCallback(
    (panelId: "party" | "initiative" | "tools", direction: "up" | "down") => {
  ...
  const movePanelInOrder = useCallback(
    (current: string[], draggedId: string, targetId: string) => {
  ...
  const handleDropLeftMasterPanel = useCallback(
    (targetId: "admin" | "tokens") => {
      if (!draggedMasterPanel) return;
      if (gmPanelOrder.includes(draggedMasterPanel as "admin" | "tokens")) {
        setGmPanelOrder(
          (current) =>
            movePanelInOrder(current, draggedMasterPanel, targetId) as Array<
              "admin" | "tokens"
            >,
        );
      } else if (
  ...
  const handleDropRightMasterPanel = useCallback(
    (targetId: "party" | "initiative" | "tools") => {
      if (!draggedMasterPanel) return;
      if (
        gmRightPanelOrder.includes(
          draggedMasterPanel as "party" | "initiative" | "tools",
                  ...
                  >
                    <div className="flex cursor-grab flex-wrap items-center gap-2 rounded-2xl border border-white/8 bg-slate-950/40 px-3 py-2 text-xs text-slate-300 active:cursor-grabbing">
                      <span className="font-medium text-white">
                        Панель мастера:{" "}
                        <AdaptiveLabel
                          full={panelId === "admin" ? "админ-панель" : "токены"}
                      ...
                      <button
                        ...
                        disabled={index === gmPanelOrder.length - 1}
                        className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                      >
                        ↓
                      </button>
                      ...
                      >
                        ...
                        <div className="mt-4 rounded-2xl border border-white/10 p-3 text-sm text-slate-300">
                          ...
                          <div className="mt-2 text-xs text-slate-400">
                            Любой формат в диапазоне от {MIN_GRID}×{MIN_GRID} до{" "}
                            {MAX_GRID}×{MAX_GRID}.
                          </div>
                        </div>
                        <button
              ...
              >
                {role === "gm" ? (
                  <div className="flex cursor-grab flex-wrap items-center gap-2 rounded-2xl border border-white/8 bg-slate-950/40 px-3 py-2 text-xs text-slate-300 active:cursor-grabbing">
                    ...
                    <button
                      type="button"
                      onClick={() => handleMoveGmRightPanel("party", "up")}
                      disabled={gmRightPanelOrder.indexOf("party") === 0}
                      className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                    >
                      ↑
                    </button>
                    <button
                      type="button"
                      onClick={() => handleMoveGmRightPanel("party", "down")}
                      disabled={
                        gmRightPanelOrder.indexOf("party") ===
                        gmRightPanelOrder.length - 1
                      }
                      className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                    >
                      ↓
              ...
              >
                {role === "gm" ? (
                  <div className="flex cursor-grab flex-wrap items-center gap-2 rounded-2xl border border-white/8 bg-slate-950/40 px-3 py-2 text-xs text-slate-300 active:cursor-grabbing">
                    ...
                    <button
                      type="button"
                      onClick={() => handleMoveGmRightPanel("initiative", "up")}
                      disabled={gmRightPanelOrder.indexOf("initiative") === 0}
                      className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                    >
                      ↑
                    </button>
                    <button
                      type="button"
                      onClick={() =>
                        handleMoveGmRightPanel("initiative", "down")
                      }
                      disabled={
                        gmRightPanelOrder.indexOf("initiative") ===
                        gmRightPanelOrder.length - 1
                      }
                      className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                    >
                  ...
                  <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
                    ...
                    >
                      <div className="flex cursor-grab flex-wrap items-center gap-2 rounded-2xl border border-white/8 bg-slate-950/40 px-3 py-2 text-xs text-slate-300 active:cursor-grabbing">
                        ...
                        <button
                          type="button"
                          onClick={() => handleMoveGmRightPanel("tools", "up")}
                          disabled={gmRightPanelOrder.indexOf("tools") === 0}
                          className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                        >
                          ↑
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            handleMoveGmRightPanel("tools", "down")
                          }
                          disabled={
                            gmRightPanelOrder.indexOf("tools") ===
                            gmRightPanelOrder.length - 1
                          }
                          className="rounded-full border border-white/10 px-2 py-1 disabled:opacity-40"
                        >
