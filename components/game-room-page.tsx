...
export function GameRoomPage({ roomId }: { roomId: string }) {
  ...
  const [mainMapSnapshot, setMainMapSnapshot] = useState<{
    ...
    tokens: initialTokens.map(normalizeToken),
  });
  ...
  }, [activeSavedMapId, mapName, mapState, tokens]);
    ...
    (tokenId: string) => (event: ReactPointerEvent<HTMLButtonElement>) => {
      ...
      if (token.linkedMapId) {
        ...
        if (linkedPreset) {
          ...
          setTokens((linkedPreset.tokens ?? []).map(normalizeToken));
  ...
  const handleLoadSavedMap = (preset: SavedMapPreset) => {
    ...
    setTokens((preset.tokens ?? []).map(normalizeToken));
  ...
  const handleReturnToMainMap = useCallback(() => {
    ...
    setMapName(mainMapSnapshot.mapName);
    setMapState(mainMapSnapshot.mapState);
    setTokens(mainMapSnapshot.tokens.map(normalizeToken));
    setGridColsInput(String(mainMapSnapshot.mapState.cols));
    setGridRowsInput(String(mainMapSnapshot.mapState.rows));
  ...
  const handleSetCurrentAsMainMap = useCallback(() => {
    setMainMapSnapshot({ mapName, mapState, tokens: tokens.map(normalizeToken) });
    addJournalEntry("map", `Текущая сцена «${mapName}» назначена основной.`);
  }, [addJournalEntry, mapName, mapState, tokens]);
        ...
        <section className="space-y-4">
          ...
          {role === "gm" ? (
            <div className="space-y-4">
              ...
              <div className="card flex flex-wrap items-center gap-2 px-3 py-2 text-xs text-slate-300">
                <span className="text-slate-400">Игроки (быстрый доступ):</span>
                {tokens
                  ...
                  .map((token) => (
                    <button
                      ...
                      className={`rounded-full border px-3 py-1.5 ${selectedTokenId === token.id ? "border-cyan-300/60 bg-cyan-500/20 text-cyan-100" : "border-white/10 text-slate-200"}`}
                    >
                      {token.name}
                    </button>
                  ))}
